...
namespace Libgit2.Internals
{
    unsafe partial class NativeMethods
    {
        ...
        public static extern result git_attr_foreach(git_repository* repository, AttributeCheckFlags flags, byte* path, git_attr_foreach_cb callback, void* payload);

        [DllImport(DllName, CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Ansi, EntryPoint = "git_attr_get")]
        public static extern result git_attr_get(byte** value_out, git_repository* repository, AttributeCheckFlags flags, byte* path, byte* name);

        [DllImport(DllName, CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Ansi, EntryPoint = "git_attr_get_many")]
        public static extern result git_attr_get_many(byte** values_out, git_repository* repository, AttributeCheckFlags flags, byte* path, UIntPtr count, byte** names);

        [DllImport(DllName, CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Ansi, EntryPoint = "git_attr_value")]
        public static extern AttributeType git_attr_value(byte* attribute_name);
    }
